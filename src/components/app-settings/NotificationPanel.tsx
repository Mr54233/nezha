...
export function NotificationPanel({ settings, onChange, projectPath }: NotificationPanelProps) {
  const { t } = useI18n();
  const [hooksConsent, setHooksConsent] = useState(false);

  useEffect(() => {
    if (!projectPath) return;
    invoke<{ agent: { hooks_consent?: boolean | null } }>("read_project_config", {
      projectPath,
    }).then((cfg) => {
      const val = cfg.agent?.hooks_consent;
      setHooksConsent(val === true);
    }).catch(() => {});
  }, [projectPath]);

  async function toggleHooksConsent() {
    const next = !hooksConsent;
    try {
      await invoke("set_hooks_consent", { projectPath, consent: next });
      setHooksConsent(next);
    } catch { /* consent toggle failed */ }
  }
  ...
  return (
    <div style={{ ...s.settingsBody, display: "flex", flexDirection: "column", gap: 8, padding: 20 }}>
      <ToggleRow
        label={t("notif.hooksConsent")}
        description={t("notif.hooksConsentDesc")}
        checked={hooksConsent}
        onChange={() => toggleHooksConsent()}
      />
      <ToggleRow
      ...
      <CheckboxRow
        ...
        disabled={disabled}
        onChange={() => updateType("input_required", !settings.types.input_required)}
      />
    </div>
  );
}
